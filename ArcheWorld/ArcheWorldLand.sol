...
contract BoraV2ERC721 is ERC721Full, ERC721Burnable, Ownable {
    ...
    uint256[5] private _mintPrice;                  // 1 KLAY = 1000000000000000000
    ...
    function setConfig(uint256 antibotInterval, uint256 mintLimitPerSale, uint256[5] memory mintPrice) public onlyOwnerOrAdmin {
        _antibotInterval = antibotInterval;
        _mintLimitPerSale = mintLimitPerSale;
        _mintPrice = mintPrice;
    }

    function configInfo() external view returns (uint256[2] memory) {
      uint256[2] memory info = [_antibotInterval, _mintLimitPerSale];
      return info;
    }

    function mintPriceInfo() external view returns (uint256[5] memory) {
        return _mintPrice;
    }

    function setPublicMintEnabled(bool useFlag) public onlyOwnerOrAdmin {
        publicMintEnabled = useFlag;
    }

    function isPublicMintEnabled() public onlyOwnerOrAdmin returns (bool) {
    ...
    function setWhitelistMintEnabled(bool useFlag) public onlyOwnerOrAdmin {
        whitelistMintEnabled = useFlag;
    }

    function isWhitelistMintEnabled() public onlyOwnerOrAdmin returns (bool) {
