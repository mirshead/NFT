...
contract W2B721 is ERC721Full, ERC721Burnable, Ownable {
    ...
    function modifyWhitelist(address targetAddress, bool addToWhitelist) public onlyOwnerOrOperator {
        if(addToWhitelist){
    ...
    function modifyMultipleWhitelist(address[] memory targetAddress, bool addToWhitelist) public onlyOwnerOrOperator {
        for (uint256 i = 0; i < targetAddress.length; i++) {
            if(addToWhitelist){
                Roles.add(whitelistedRole, targetAddress[i]);
            } else{
                Roles.remove(whitelistedRole, targetAddress[i]);
            }
            emit WhitelistModified(targetAddress[i], addToWhitelist);
        }
    }
