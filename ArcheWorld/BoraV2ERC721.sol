...
contract BoraV2ERC721 is ERC721Full, ERC721Burnable, Ownable {
    ...
    Roles.Role private whitelistedRole;
    ...
    event WhitelistModified(address indexed targetAddress, bool addedToWhitelist);
    ...
    function mintWithUri(address to, uint256 tokenId, string memory uri) public onlyOwnerOrAdmin returns (bool) {
    ...
    function mintWithUri(address to, uint256 tokenId, string memory uri, uint256 serviceId) public onlyOwnerOrAdmin returns (bool) {
        mintWithUri(to, tokenId, uri);
    ...
    function modifyWhitelist(address targetAddress, bool addToWhitelist) public onlyOwnerOrAdmin {
        if(addToWhitelist){
            Roles.add(whitelistedRole, targetAddress);
        } else{
            Roles.remove(whitelistedRole, targetAddress);
        }
        emit WhitelistModified(targetAddress, addToWhitelist);
    }
    
    function modifyMultipleWhitelist(address[] memory targetAddress, bool addToWhitelist) public onlyOwnerOrAdmin {
        for (uint256 i = 0; i < targetAddress.length; i++) {
            modifyWhitelist(targetAddress[i], addToWhitelist);
        }
    }
    ...
    function setUseWhitelisted(bool useFlag) public onlyOwnerOrAdmin {
